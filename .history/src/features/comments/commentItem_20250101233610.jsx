...
function CommentItem({ comment, deleteComment }) {
  ...
  return (
    <li className="w-full flex flex-col-reverse items-start justify-center bg-gray-200 p-4 rounded-lg my-2 hover:scale-105 transition-all duration-300">
      <div className="w-full lg:flex-row flex-col  flex items-center justify-between p-2 relative">
        <img
          src={info.avatar_url}
          alt="profile-image"
          className="w-7 h-7 rounded-full absolute -top-5 -left-8 z-30 border border-lime-500"
        />
        <button
        ...
        <span className="w-fit px-3 bg-black rounded-2xl text-white">
          ...
          {comment.type === "bug-issue" ? "Bug issue" : ""}
          {comment.type === "feature-request" ? "Feature request" : ""}
        </span>
      </div>{" "}
