...
function RepoItem({ repoData }) {
  ...
  return (
    <article className="w-1/4 h-44 border rounded-xl flex bg-white justify-between flex-col p-3 hover:filter hover:invert transition-all duration-300 hover:scale-110">
      ...
      <div className="w-full flex flex-row items-center justify-between gap-6">
        ...
        <button
          className="w-1/2 h-5 rounded bg-gradient-to-r from-zinc-700 to-rose-800 text-xs text-white font-bold uppercase"
          onClick={() => navigate(`/repo/${repoData.id}`)}
        >
          Add Comment
        </button>
      </div>
